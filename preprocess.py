...
def build_vocab(data, min_token_instances, vocab_output, verbose=True):
  ...
  with open(vocab_output, 'w') as f:
    json.dump(sorted(token_counter.items(),key=itemgetter(1),reverse=True), f)
    #json.dump(list(vocab), f)

  with open('token_counter.json', 'w') as f:
    json.dump(sorted(token_counter.items(),key=itemgetter(1),reverse=True), f)
    #json.dump(list(vocab), f)
...
def words_preprocess(phrase):
  ...
  return str(phrase).lower().split()

def save_json(name, data):
  with open(name, 'w') as f:
      json.dump(data, f)

def split_filter_captions(data, max_token_length, tokens_type, min_regions, max_regions, verbose=True, split = None):
  """
  ...
  for i, img in enumerate(data):
    ...
    for j, region in enumerate(img['regions']):
    ...
    if regions_per_image == 0:# < min_regions or regions_per_image > max_regions:
      #print 'kept %d, removed %d' % (img_kept, img_removed)
      imgs_to_removed.append(img['id'])
  ...
  print('{} images will be deleted'.format(len(imgs_to_removed)))
  new_data = []
  
  for d in data:
    if not d['id'] in imgs_to_removed:
      new_data.append(d)  
  ...
  for d in data:
    for r in d['regions']:
      ...
      if d['id'] not in imgs_to_removed:
        assert False, 'Erro'
  """
...
def main(args):
  ...
  print('loading data...')
  # read in the data
  data = load_data(args.region_data)
  print('data loaded...')
  ...
  data = split_filter_captions(data, args.max_token_length, args.tokens_type, \
                               args.min_regions, args.max_regions, split = split_data)
  ...
  # build vocabulary
  vocab = build_vocab(data, args.min_token_instances, args.vocab_output) # vocab is a set()
  print('length vocab: ' + str(len(vocab)))
  token_to_idx, idx_to_token = build_vocab_dict(vocab) # both mappings are dicts
  save_json(''.join([args.region_data.split('.json')[0], '_kept', '.json']), data)
  ...
  save_json(args.json_output, json_struct)
  save_json(''.join([args.region_data.split('.json')[0], '_kept', '.json']), data)
...
if __name__ == '__main__':
  ...
  parser.add_argument('--vocab_output',
  ...
  parser.add_argument('--min_regions', default = 20, type=int)
  parser.add_argument('--max_regions', default = 50, type=int)
